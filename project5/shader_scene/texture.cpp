...
float3 NormalMap::computeNormal(int i, int j, float scale)
{
    ...
    assert(image+3*width*height > &image[  (i           *width+ j         )*components+0]);
    assert(image+3*width*height > &image[  (i           *width+(j+1)%width)*components+0]);
    assert(image+3*width*height > &image[(((i+1)%height)*width+ j         )*components+0]);

    int ndx       =   (j           *width+ i         )*components+0;
    int ndx_right =   (j           *width+((i+1)%width))*components+0;
    int ndx_up    = (((j+1)%height)*width+ i         )*components+0;

    float c  = image[ndx      ] * oneOver255;
    float cx = image[ndx_right] * oneOver255;
    float cy = image[ndx_up   ] * oneOver255;
    float2 gradient = scale*(float2(c)-float2(cx,cy));

    float3 normal = float3(gradient,1);
